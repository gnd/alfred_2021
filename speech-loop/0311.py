...
# Note: Run as `python3 0311.py 2>/dev/null` to redirect stderr
...
def recognize_engine_switch(text):
    global ENGINE
    m = re.search(r"\b(engine (normal|instruct))\b", text, re.I)
    if m:
        engine = m.group(2)
        if engine == "normal":
...
def do_with_hypothesis(hypothesis):
    if recognize_engine_switch(hypothesis):
        return
    if recognize_temperature(hypothesis):
        return
